...
#############################################################
###########  temp*preci / temp lag는 8주까지 / week df = 3,4,5,6 / year trend ns(df = 3)로 
#############################################################


##################################################################################################
######## Autocorrealtion 항 추가
################################################################################################
...
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 6) + ns(year, df = 3) + holi + auto, Jeonju_2019, 
             family=quasipoisson)


# Prediction and visualization
pred.temp <- crosspred(cb.temp, model, cen=5, by=1)
...
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 6) + ns(year, df = 3) + holi + auto, Jeongeup_2019, 
             family=quasipoisson)
...
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 6) + ns(year, df = 3) + holi + auto, Ulsan_2019, 
             family=quasipoisson)
...
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 6) + ns(year, df = 3) + holi + auto, Jeju_2019, 
             family=quasipoisson)
...
#############################################################
###########  temp*preci / temp lag는 12주까지 / week df = 3,4,5,6 / year trend ns(df = 3)로 
#############################################################
...
# 여기서 percentile은 위에서 pct에서 계산된 값 넣는 것.
par(mfrow=c(1,1))
plot(pred.temp, "overall", ylim=c(0.9,500), col=2, lwd=2, xlab="Temperature", ylab="RR",
     main="Overall cumulative association")
abline(v=5, lty=2, col="grey") # MMT minimum mortality temperature
abline(v=28, lty=2, col=2) # 99th
abline(v=-4, lty=2, col=4) # 1st
