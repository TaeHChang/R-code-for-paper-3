...
load(file = "C:/Users/Taehee/OneDrive/바탕 화면/My papers/Univ Tokyo papers_작업중/dataset/final_data_전국.RData")
...
write.csv(Jeonju_over32, "C:\\Users\\Taehee\\OneDrive\\바탕 화면\\My papers\\Univ Tokyo papers_작업중\\Calculated_data\\Jeonju_over32.csv")
...
write.csv(a, "C:\\Users\\Taehee\\OneDrive\\바탕 화면\\My papers\\Univ Tokyo papers_작업중\\Calculated_data\\a.csv")
...
Jeonju_over32$holi <- as.factor(holi$holi)
Jeongeup_over32$holi <- as.factor(holi$holi)
Ulsan_over32$holi <- as.factor(holi$holi)
Jeju_over32$holi <- as.factor(holi$holi)




#############################################################
###########  temp*preci / temp lag는 12주까지(근데 이거 에러나서 8주로 해보기) / year trend ns(df = 3)로 / group 변수 추가. 
#############################################################
...
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 7*yr) + ns(year, df = 3) , Jeonju_over32, 
             family=quasipoisson)
...
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 7*yr) + ns(year, df = 3) + holi, Jeongeup_over32, 
             family=quasipoisson)
...
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 7*yr) + ns(year, df = 3) + holi, Ulsan_over32, 
             family=quasipoisson)
...
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 7*yr) + ns(year, df = 3) + holi, Jeju_over32, 
             family=quasipoisson)
...
### 32주 ~ 52주, 그 다음해 4주까지 사용.
### 53주를 빼는게 에러를 발생시킴. 이유는 진짜 모르겠음.
Jeonju_over32 <- Jeonju_2019 %>% filter(week > 31) 
Jeongeup_over32 <- Jeongeup_2019 %>% filter(week > 31)
Ulsan_over32 <- Ulsan_2019 %>% filter(week > 31)
Jeju_over32 <- Jeju_2019 %>% filter(week > 31)

########## 여기서 Jeonju_over32는 csv에서 불러들인거 사용해야 함. 
Jeonju_over32 <- read.csv("C:\\Users\\Taehee\\OneDrive\\바탕 화면\\My papers\\Univ Tokyo papers_작업중\\Calculated_data\\Jeonju_over32.csv")
...
#############################################################
###########  temp*preci / temp lag는 8주로 해보기 / year trend ns(df = 3)로  
#############################################################
...
yr <- length(unique(Jeonju_over32$year))
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 5*yr) + ns(year, df = 3) + holi, Jeonju_over32, 
             family=quasipoisson)


# Prediction and visualization
pred.temp <- crosspred(cb.temp, model, cen=7, by=1)
pred.preci <- crosspred(cb.preci, model, cen=8, by=1)
...
plot(pred.temp, "overall", ylim=c(0.9,30), col=2, lwd=2, xlab="Temperature", ylab="RR",
     main="Overall cumulative association")
abline(v=7, lty=2, col="grey") # MMT minimum mortality temperature
abline(v=28, lty=2, col=2) # 99th
abline(v=-4, lty=2, col=4) # 1st
...
yr <- length(unique(Jeongeup_over32$year))
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 5*yr) + ns(year, df = 3) + holi, Jeongeup_over32, 
             family=quasipoisson)
...
yr <- length(unique(Ulsan_over32$year))
# 그리고 여기에는 위에서 만든 crossbasis 객체와 covariates를 넣고 모델을 적합한다.
model <- glm(cases ~ cb.temp + cb.preci + ns(week, df = 5*yr) + ns(year, df = 3) + holi, Ulsan_over32, 
             family=quasipoisson)


# Prediction and visualization
pred.temp <- crosspred(cb.temp, model, cen=9, by=1)
pred.preci <- crosspred(cb.preci, model, cen=8, by=1)
...
plot(pred.temp, "overall", ylim=c(0.9,30), col=2, lwd=2, xlab="Temperature", ylab="RR",
     main="Overall cumulative association")
abline(v=9, lty=2, col="grey") # MMT minimum mortality temperature
abline(v=28, lty=2, col=2) # 99th
abline(v=0, lty=2, col=4) # 1st
...
### 참고용 histogram. 예시로 사용하는 지역 별로 temperature, precipitation
ggplot(data = Jeonju_over32, aes(x = tmean)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()
ggplot(data = Jeonju_over32, aes(x = total_preci)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()

ggplot(data = Jeongeup_over32, aes(x = tmean)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()
ggplot(data = Jeongeup_over32, aes(x = total_preci)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()

ggplot(data = Ulsan_over32, aes(x = tmean)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()
ggplot(data = Ulsan_over32, aes(x = total_preci)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()

ggplot(data = Jeju_over32, aes(x = tmean)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()
ggplot(data = Jeju_over32, aes(x = total_preci)) + geom_histogram(binwidth = 0.4, col = "blue", fill = "#33FFFF", alpha = 0.5) + 
  theme_bw()
